$(document).ready(function () {
    ...
    // delete banners
    $(document).on('click', '.delete_banners', function () {
        let id = $(this).data('id'); 

        var url = window.APP_URLS.deletebanners.replace(':id' , id);
        if (confirm('Are you sure you want to delete this banners?')) {
            $.ajax({
...
function validateAndPreviewImage() {
        const input = document.getElementById("banners_image");
        const file = input.files[0];
        const errorSpan = document.getElementById("span_banners_image");
        const previewImg = document.getElementById("preview_banners_image");
        const bannerId = document.getElementById("span_banners_image_id")?.value;
