$(document).ready(function () {
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".mobile-back-btn", function (e) {
            ...
            $(this)
                .closest(".dropdown-submenu, .nav-item.dropdown")
                .removeClass("show");
        });
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (mapContainer) {
        const observer = new IntersectionObserver(
            ...
            { threshold: 0.3 },
        ); // Jab 30% map dikhe tab trigger hoga
    ...
    if (cursor) {
        ...
        document.addEventListener("mouseover", (e) => {
            if (
                e.target.closest("a, button, .com_btn, input, textarea, select")
            ) {
    ...
    if (prodSection) {
        ...
        function animateProdCards() {
            if (window.innerWidth <= 1024) return;
            currentProdProgress +=
                (targetProdProgress - currentProdProgress) * 0.1; // Smooth interpolation
            ...
            cards.forEach((card, index) => {
                ...
                let cardProgress =
                    (currentProdProgress - start) / (end - start);
    ...
    const counterSection1 = document
    ...
    const counterSection2 = document
